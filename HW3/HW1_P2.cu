		...
		bool useDevice=false, dim3 blockSize=dim3(1, 1))
{
	...
	{
		// TODO
		for( int outPixelRow = 0; outPixelRow< height; outPixelRow++)
		{
			for (int outPixelCol = 0; outPixelCol< width; outPixelCol++)
			{
				float3 outPixel = make_float3(0, 0, 0);
				for ( int filterRow = 0; filterRow < filterWidth; filterRow ++)
				{
					for (int filterCol = 0; filterCol < filterWidth; filterCol++)
					{
						...
						int inPixelRow = (outPixelRow - filterWidth/2) + filterRow;
						int inPixelCol = (outPixelCol - filterWidth/2) + filterCol;
				...
				outPixels[outPixelRow*width + outPixelCol] = make_uchar3(outPixel.x, outPixel.y, outPixel.z); 
			}
		}
	}
	else // Use device
	{
		...
		size_t nOutBytes = width*height*sizeof(uchar3);
