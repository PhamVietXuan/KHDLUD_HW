...
__global__ void blurImgKernel1(uchar3 *inPixels, int width, int height,
  ...
  for (int fi = 0; fi < filterWidth; fi++)
    for (int fj = 0; fj < filterWidth; fj++) {
      int idx = fi * filterWidth + fj;
      int inPixelRow = px - filterWidth / 2 + fi;
      int inPixelCode = py - filterWidth / 2 + fj;

      inPixelRow = max(0, min(inPixelRow, height - 1));
      inPixelCol = max(0, min(inPixelCol, width - 1));

      out.x += filter[idx] * inPixels[inPixelRow * width + inpixelCol].x;
      out.y += filter[idx] * inPixels[inPixelRow * width + inpixelCol].y;
      out.z += filter[idx] * inPixels[inPixelRow * width + inpixelCol].z;
    }
  int i = px * width + py;
  outPixels[i] = make_uchar3(out.x, out.y, out.z);
}
